 ...
 * line uncommented and you perform the verification from the ADC buffer to the
 * detector, for a more thorough test. Make sure to leave the line below
 * commented until you have completely coded detector.c and buffer.c,
 * otherwise this file will not compile.
 ******************************************************************************/
//#define ADC_THROUGH_DETECTOR

#include <math.h>
#include <stdio.h>

#ifdef ADC_THROUGH_DETECTOR
#include "detector.h"
#include "buffer.h"
#define ADC_INTEGER_MIN_VALUE 0
#define ADC_INTEGER_MAX_VALUE 4095
#endif
...
uint16_t filterTest_getDecimationValue(void) {
...
bool filterTest_decimatingFirFilter(void) {
...
buffer_data_t computeAdcBufferInput(uint16_t freqTick,
                               uint16_t currentPeriodTickCount) {
  buffer_data_t adcValue;
  ...
  if (freqTick < ONE_HALF(currentPeriodTickCount)// The period has two halves: the -1.0 part and the 1.0 part.
      adcValue = ADC_INTEGER_MIN_VALUE; // The first half of the period is a minimum value.
  else
    adcValue = ADC_INTEGER_MAX_VALUE; // The second half of the period is a maximum value.
  return adcValue;
}
#endif
...
#ifdef ADC_THROUGH_DETECTOR
  detector_init(); // Use the detector to invoke the filters.
  buffer_init(); // Use the ADC buffer to provide data to the filters.
#endif
  if (printMessageFlag) {
  ...
  for (uint16_t testPeriodIndex = 0;
    ...
    while (
      ...
      for (uint16_t freqTick = 0; freqTick < currentPeriodTickCount;
        ...
        buffer_pushover(adcValue); // Put the data into the ADC buffer.
#else
        double filterValue;
        ...
        if (plotInputFlag &&
          ...
          yValues[totalTickCount] =
              filterValue +
              FILTER_TEST_INPUT_OFFSET; // The offset ensures that the input is
...
void filterTest_fillQueue(queue_t *q, queue_data_t fillValue) {
  queue_size_t n = queue_size(q);
  for (queue_index_t i = 0; i < n; i++) {
...
#ifdef ADC_THROUGH_DETECTOR
  detector_init(); // Use the detector.
  buffer_init(); // Use ADC buffer to provide data to the detector.
#endif
  if (printMessageFlag) {
...
#ifdef ADC_THROUGH_DETECTOR // From ADC buffer to detector.
        buffer_data_t adcValue;
        adcValue = computeAdcBufferInput(freqTick, currentPeriodTickCount);
        buffer_pushover(adcValue); // Put the data into the ADC buffer.
...
#ifdef ADC_THROUGH_DETECTOR // Run the detector to run the filters.
    bool interruptsEnabled = false; // Need to tell the detector that interrupts
                                    // are not currently enabled.
    detector(interruptsEnabled, false); // Run the detector so that it runs the
                                        // decimating FIR and IIR filters.
...
bool filterTest_runIirBAlignmentTest(uint16_t filterNumber,
  ...
  bool success = true;                       // Be optimistic.
  filterTest_fillQueue(filter_getYQueue(), 0.0); // zero-out the yQueue.
  filterTest_fillQueue(filter_getZQueue(filterNumber),
...
uint16_t filterTest_getIirACoefficientArrayStartingIndex(void) {
...
bool filterTest_runIirAAlignmentTest(uint16_t filterNumber,
  ...
  bool success = true; // Be optimistic.
  filterTest_fillQueue(filter_getYQueue(),
...
double filterTest_randomValue0To1(void) {
...
void filterTest_fillQueueWithRandomValues(queue_t *q) {
  ...
  for (queue_index_t i = 0; i < n; i++)
    ...
    queue_overwritePush(q, filterTest_randomValue0To1());
}
...
double filterTest_computeGoldenPowerValue(queue_t *q) {
  ...
  // power value.
  queue_index_t queueSizeOffset = 0;
#ifdef FILTER_TEST_STORE_OLD_VALUE_IN_QUEUE
  queueSizeOffset = 1;
#endif
  // Iterate over all elements.
  for (queue_index_t i = queueSizeOffset; i < n; i++) {
    double elementValue = queue_readElementAt(q, i); // Read from queue.
...
// Loop over the incremental test this many times.
#define TEST_INCREMENTAL_LOOP_COUNT 3000
...
bool filterTest_runPowerTest(void) {
  ...
  // Perform many incremental power computations.
...
//        to a legal ADC value. buffer_pushover(adcValue); // Send the
